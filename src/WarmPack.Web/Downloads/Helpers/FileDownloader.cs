// tomado de https://www.codeproject.com/Articles/14405/Easy-to-use-resumable-NET-file-downloader
...
namespace CodeProject.Downloader
{
    ...
    public class FileDownloader
    {
        ...
        private const int downloadBlockSize = 1024 * 16;
        ...
        public event DownloadExceptionHandler ExceptionCatched;
        ...
        public void Download(string url, string destFolder)
        {
            ...
            try
            {
                ...
                using (var f = File.Open(this.downloadingTo, FileMode.Append, FileAccess.Write))
            ...
            {
                RaiseDownloadException(ex);
                //throw ex;
                //SynchronizationContext.Current.Send(state => { throw ex; }, null);
            }
            finally
            {
                if (data != null)
        ...
        private void SaveToFile(byte[] buffer, int count, FileStream fs)
        {
            FileStream f = null;
            ...
            {
                throw new ArgumentException(
                    String.Format("Error trying to save file \"{0}\": {1}", "", e.Message), e);
            }
            finally
            {
                if (f != null)
        ...
        private void RaiseDownloadException(Exception exception)
        {
            if (this.ExceptionCatched != null)
                this.ExceptionCatched(this, new DownloadExceptionEventArgs(exception));
        }
    }
    ...
    public delegate void DownloadExceptionHandler(object sender, DownloadExceptionEventArgs e);
